...
void 
ol_keybinder_bind (const char           *keystring,
		       GnomeDoBindkeyHandler  handler,
		       gpointer              user_data)
{
	Binding *binding;
...
void
ol_keybinder_unbind (const char           *keystring, 
			 GnomeDoBindkeyHandler  handler)
{
	GSList *iter;
 ...
 */
gboolean
ol_keybinder_is_modifier (guint keycode)
{
	gint i;
	gint map_size;
...
guint32
ol_keybinder_get_current_event_time (void)
{
	if (processing_event) 
