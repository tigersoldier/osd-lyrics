...
static gboolean ol_player_amarok2_init_dbus ();
...
ol_player_amarok2_get_metadata ()
{
  ...
  if (proxy == NULL)
    if (!ol_player_amarok2_init_dbus ())
      return FALSE;
...
ol_player_amarok2_get_music_info (OlMusicInfo *info)
{
  ...
  if (info == NULL)
    return FALSE;
  if ((data_list = ol_player_amarok2_get_metadata ()) != NULL)
  {
    if (info->artist)
      g_free (info->artist);
...
ol_player_amarok2_get_music_length (int *len)
{
  ...
  if ((metadata = ol_player_amarok2_get_metadata ()) != NULL)
  {
...
ol_player_amarok2_init_dbus ()
{
  ...
  if (proxy == NULL)
  {
    ...
    g_signal_connect (proxy, "destroy", G_CALLBACK (ol_player_amarok2_proxy_destroy_handler), NULL);
  }
  return TRUE;
}
